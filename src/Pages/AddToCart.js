...
const AddToCart = (props) => {
  ...
  const addToCartData = useSelector((state) => state.cartUi.products.find(product => product.id === productId));

  const handleAddToCart = () => {
    const item = cart_items.find(
      (item) => item?.id === addToCartData.id
    );

    if (item && item?.cartCount + product_item > addToCartData?.stock) {
